...
public class ImageViewerPopupView extends BasePopupView implements OnDragChangeListener, View.OnClickListener {
    ...
    private void addOrUpdateSnapshot() {
        ...
        setupPlaceholder();
        if(srcView.getDrawable() == null || srcView.getDrawable().getConstantState() == null){
            snapshotView.setImageDrawable(srcView.getDrawable());
        }else{
            snapshotView.setImageDrawable(srcView.getDrawable().getConstantState().newDrawable().mutate());
        }
    }
    ...
    public void doDismissAnimation() {
        ...
        pager.setVisibility(INVISIBLE);
        // snapshotView更新为当前的Drawable
        PhotoViewAdapter adapter = (PhotoViewAdapter) pager.getAdapter();
        PhotoView curPhotoView = null;
        if(adapter != null){
            curPhotoView = adapter.getCurPhotoView();
        }
        if(curPhotoView != null){
            snapshotView.setImageDrawable(curPhotoView.getDrawable());
        }else{
            imageLoader.loadImage(position,urls.get(position),snapshotView);
        }
        snapshotView.setVisibility(VISIBLE);
    ...
    public class PhotoViewAdapter extends PagerAdapter {
        ...
        private PhotoView mCurPhotoView;

        public PhotoView getCurPhotoView() {
            return mCurPhotoView;
        }
        ...
        public void setPrimaryItem(@NonNull ViewGroup container, int position, @NonNull Object object) {
            mCurPhotoView = (PhotoView) object;
        }
    }
